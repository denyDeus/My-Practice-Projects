...
WORKBOOK_PATH = Path(__file__).resolve().parent / 'attendance.xlsx'
...
# staff mail ids by subject
staff_mails = {
    1: 'deusdenis40@gmail.com',
    2: 'goodluckgyunda@gmail.com',
    3: 'goodluckgyunda@gmail.com'
}

# sender email and Gmail App Password
sender_email = 'deusdenis40@gmail.com'
sender_password = 'dqgmesiomowczmrn'
...
def savefile():
    try:
        ...
        print(f"Could not save to the workbook because it is locked or in use: {exc}")
...
def check(no_of_days, row_num, b):
    ...
    for student in range(0, len(row_num)):
        ...
        if l2 != "" and len(l3) != 0:
            ...
            staff_id = staff_mails.get(b)
            if staff_id:
                mailstaff(staff_id, msg2)  # mail to staff
            else:
                print(f"No staff email configured for subject code {b}")

# for students
def mailstu(li, msg):
    from_id = sender_email
    pwd = sender_password
    s = None
    try:
        ...
        print("Email login failed. Check the sender email/password or use a Gmail App Password.")
        print(exc)
    except smtplib.SMTPException as exc:
        print(f"Email could not be sent: {exc}")
    finally:
        if s is not None:
            try:
                s.quit()
